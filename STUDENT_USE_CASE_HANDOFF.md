...
| 32 | Student payment status and checkout | Student | **Next.** Add a payment status/history surface from `GET /api/payments`. For a confirmed booking, start `POST /api/payments/intents` and hand its `client_secret` to a real Stripe Flutter payment-sheet integration. Never present a payment as successful until a refresh shows webhook-confirmed `succeeded` status. |
...
3. **Flutter UI wiring.** Use case 31 now provides the student notification inbox. Continue with use case 32 (payment status/real Stripe checkout), then 33 (announcement feed), 34 (warden dashboard), and 35 (owner dashboard). The backend and `lib/api.dart` already expose the required endpoints; retain their server-side authorization boundaries.
