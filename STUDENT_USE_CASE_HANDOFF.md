...
## Post-Roadmap Flutter Use Cases

The original 1-30 roadmap established and verified the server contracts. The following UI use cases continue the product journey and must use those existing contracts rather than creating alternate client-side state or new API aliases.

| # | Use case | Primary actor | Status and definition of done |
|---|---|---|---|
| 31 | Student notification inbox | Student | **Complete.** [notifications_screen.dart](lib/screens/notifications_screen.dart) loads the authenticated student's `GET /api/notifications` inbox, supports pull-to-refresh, handles loading/error/empty states, distinguishes unread items, and marks an item read through `PATCH /api/notifications/:id/read` when opened. The signed-in hostel discovery header exposes it through a notification icon. Focused Flutter analysis passes with no issues for the new screen; the hosting discovery screen retains unrelated pre-existing style infos. |
| 32 | Student payment status and checkout | Student | **Complete (code/config ready).** [payments_screen.dart](lib/screens/payments_screen.dart) lists authenticated `GET /api/payments` records, exposes payment history from My Bookings, and offers a PaymentSheet action only for a confirmed booking without an active payment. `flutter_stripe` is initialized in `main.dart` only when the app is launched with `--dart-define=STRIPE_PUBLISHABLE_KEY=pk_test_...`; the screen creates an intent through `POST /api/payments/intents`, presents Stripe PaymentSheet, then refreshes server state and only displays `succeeded` after the webhook does. The Node intent now requests Stripe automatic payment methods for PaymentSheet compatibility. **Still required before production:** configure a real Stripe test-mode publishable key and secret/webhook keys, then complete one device-level test payment and verify webhook settlement. |
| 33 | Student announcement feed | Student | **Next.** Add a read-only feed using `GET /api/announcements`, scoped by the backend to the student's active bookings/favorites. Keep notification preferences as the delivery control; do not expose any client-side audience bypass. |
| 34 | Warden operational dashboard | Warden | **Next.** Replace prototype/static warden views with assignment-scoped booking and complaint queues, available-room lookup, room assignment, and check-in/check-out controls using the existing `Api` methods. Do not add hostel IDs to client requests as an authorization mechanism. |
| 35 | Owner operational dashboard | Owner | **Next.** Connect owner hostel management, booking decisions, complaint triage, occupancy, rooms, and warden assignment views to the owner-scoped endpoints already in `Api`. Privileged account provisioning remains an admin-controlled operation, not a public sign-up workflow. |
...
3. **Flutter UI wiring.** Use cases 31 (notification inbox) and 32 (Stripe PaymentSheet/status UI) are now implemented. Continue with 33 (announcement feed), 34 (warden dashboard), and 35 (owner dashboard). The backend and `lib/api.dart` already expose the required endpoints; retain their server-side authorization boundaries.
...
All 30 roadmap use cases now have this level of validation at the API layer (`node scripts/run_db_integration_tests.js` runs 5 disposable-database regressions covering student, owner, warden, room/stay, and payment flows). Post-roadmap Flutter use case 31 is complete. The product is not yet launch-ready: see the post-roadmap UI table and "Handoff: What A New Agent Should Do Next" above for remaining UI and deployment-operations work.
