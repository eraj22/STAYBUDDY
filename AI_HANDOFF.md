...
## 2026-07-20: Parent Links Phase Started

The working copy now includes `migrations/008_parent_links.sql` and the first parent-account API slice. It adds the `parent` role, administrator-only parent provisioning, student-created 72-hour parent invitations, authenticated parent acceptance, immediate student revocation, linked-child listing, and a parent child-overview endpoint. Full parent access is derived only from an active `parent_student_links` row; client-supplied relationship IDs never authorize access. The disposable regression is `test/parent_authorization.integration.test.js` and is run by `node scripts/run_db_integration_tests.js` with test-only invitation token delivery enabled.

Attendance, emergency escalation, and administrator verification are not yet implemented. Do not start their notification integrations until the parent-link regression passes.

## 2026-07-21: Attendance and Curfew Phase Completed

Migration `009_attendance_curfew.sql` adds hostel curfew settings, scanner credentials, resident RFID/biometric identifiers, immutable attendance events, correction audit records, and attendance exceptions. `attendance_routes.js` supplies owner configuration/credential routes, warden enrollment/history/correction/curfew-evaluation routes, authenticated scanner ingestion with idempotency keys, and linked-parent attendance history. Scanner events and curfew exceptions create preference-gated in-app notifications for linked parents. The disposable regression is `test/attendance.integration.test.js`; it proves idempotent scanner ingestion plus warden and parent isolation. Apply migrations `008` and `009` explicitly before using these routes in a non-disposable database.
...
The payment status/checkout UI is now implemented in `../lib/screens/payments_screen.dart` and entered from `../lib/screens/my_bookings_screen.dart`. A confirmed booking with no pending/succeeded payment can call `POST /api/payments/intents`, open the `flutter_stripe` PaymentSheet using the returned `client_secret`, and reload server payment history. `main.dart` configures the SDK only from `--dart-define=STRIPE_PUBLISHABLE_KEY=pk_test_...`; never commit the publishable key or secret key. The Node intent includes `automatic_payment_methods[enabled]=true`, which PaymentSheet needs. The UI deliberately reports a submitted payment as pending until `GET /api/payments` reflects webhook-confirmed `succeeded`. Focused Flutter analysis of `main.dart`, `my_bookings_screen.dart`, and `payments_screen.dart` passed clean on 2026-07-18; `node scripts/run_db_integration_tests.js` remained 5/5 passing.

The next UI contracts to wire are the student announcement feed (`GET /api/announcements`), the warden operational queue and move workflow, and owner operational management. Keep all authorization server-derived: UI state and client-supplied hostel IDs must never decide what a role may read or mutate.
